...
# jtab instruction is similar to ret instruction. no need to predict 
# it should stall in fetch and bubble up in decode stage 
# 
# firstly like in seq-full.hcl 
# in fetch part added jtab in instr_valid, need_regids and         
# need_valC, since it is valid we need valC and regids.   
#          
# in decode part we need srcB from D_rB 
# and setted D_rA for dstE 
# 
# in execute part we need to sum valC and valB to calculate memory 
# position C(%rB).
#                                   
# in memory stage we need to read jump address so added    
# jtab in mem_read, read from previously calculated                
# valE (valC+valB).
#
# in pipeline register control part we have to wait for the address 
# coming from memory.
# to implement this stall in fetch and bubble while jtab in pipe
# added F_stall = IJTAB in { D_icode, E_icode, M_icode } 
# and D_bubble = IJTAB in { D_icode, E_icode, M_icode };
# 
# f_pc come from W_valM like ret
#
...
word d_dstM = [
	D_icode in { IMRMOVQ, IPOPQ, IJTAB } : D_rA;
	1 : RNONE;  # Don't write any register
];
...
bool F_stall =
	...
	# stall at fetch while jtab pass thru pipeline 
...
bool D_bubble =
  ...
  IRET in { D_icode, E_icode, M_icode } || 
  	# Stalling at fetch while jtab passes through pipeline
	# but not condition for a load/use hazard
  IJTAB in { D_icode, E_icode, M_icode };
